...
export interface TrainingContext {
  ...
  targetResult?: 'win' | 'draw' | 'loss'
}
export interface EndgameTrainingMove {
  uci: string
  source: 'tablebase' | 'stockfish' | 'scripted' | 'legal'
  explanation?: string
}
export interface EndgameTrainingOpponent {
  getMove(position: string, context: TrainingContext): Promise<EndgameTrainingMove | null>
}
...
export class TablebaseOpponent implements EndgameTrainingOpponent {
  ...
  async getMove(position: string): Promise<EndgameTrainingMove | null> {
    ...
    const move = result?.lances.find(
      (candidate) =>
        parseUci(candidate.uci) !== null &&
        legalMoves(position).some((legal) => legal.uci === candidate.uci),
    )
    return move
      ? { uci: move.uci, source: 'tablebase', explanation: 'Defesa escolhida pela tablebase.' }
      : null
  }
}
...
export class ScriptedTechniqueOpponent implements EndgameTrainingOpponent {
  ...
  async getMove(position: string, context: TrainingContext): Promise<EndgameTrainingMove | null> {
    ...
    if (candidate && legalMoves(position).some((move) => move.uci === candidate))
      return { uci: candidate, source: 'scripted' }
    const fallback = legalMoves(position)
      .map((move) => move.uci)
      .sort()[0]
...
export function aplicarMovimentoDoOponente(
  position: string,
  move: EndgameTrainingMove,
): string | null {
