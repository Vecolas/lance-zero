...
describe('Roadmap V3 e elegibilidade de revisão', () => {
  ...
  it('ensino concluído mantém o check quando a revisão vence', () => {
    ...
    const taught = {
      ...
      exposureCount: 1,
      lastTaughtAt: '2026-01-01T00:00:00.000Z',
    }
    const node = ROADMAP_DEFINITION.nodes.find((item) => item.skillId === 'tactics.fork')!
    const view = deriveRoadmapNode(node, {
      ...userLearningStateFromSkill(taught, node.learningObjectId),
      stage: 'review',
    })
  ...
  it('reensino bloqueia recuperação independente sem apagar o histórico', () => {
    const state = marcarParaReensino(
      { ...criarSkillState('tactics.fork', new Date()), exposureCount: 1, stage: 'independent' },
      new Date(),
    )
  ...
  it('expõe currículo amplo e escolha de repertório sem obrigar todas as opções', () => {
    ...
    expect(ROADMAP_DEFINITION.areas.map((area) => area.id)).toEqual(
